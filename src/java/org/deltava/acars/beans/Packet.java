...
public class Packet {
	...
	/**
	 * Populates a VoiceMessage from an MVS voice packet. The packet is contained within the message's data.
	 * @param msg the VoiceMessage
	 * @throws IOException if an error occurs
	 */
	public static void parse(VoiceMessage msg) throws IOException {

		try (PacketInputStream in = new PacketInputStream(new ByteArrayInputStream(msg.getData()))) {
	...
	public static byte[] rewrite(VoiceMessage vmsg) throws IOException {
		...
		if (hdrSize < 1) throw new IllegalStateException("MVS Header size unknown");
