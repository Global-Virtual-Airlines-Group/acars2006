...
public class ACARSConnection implements Serializable {
   ...
   public ACARSConnection(long cid, SocketChannel sc) {
      ...
      try {
         ...
         System.out.println("Cannot set non-blocking I/O from " + remoteAddr.getHostAddress());
         try {
            sc.close();
         } catch (Exception e) {
         }
   ...
   void write(String msg) {
      ...
      if (msg.length() > oBuffer.limit())
         oBuffer = ByteBuffer.allocate(msg.length() + 16);

      // Dump the message into the buffer and write to the socket channel
      try {
         oBuffer.put(msg.getBytes());
         oBuffer.flip();
         channel.write(oBuffer);
         bytesOut += msg.length();
         lastActivityTime = System.currentTimeMillis();
      } catch (IOException ie) {
         System.out.println("Error writing to socket " + remoteAddr.getHostAddress() + " - " + ie.getMessage());
      }
